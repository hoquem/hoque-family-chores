...
class MockTaskService implements TaskServiceInterface {
  ...
  MockTaskService() {
    _taskStreamController.add(List.from(_tasks)); // Initial emission of all tasks
  ...
  Stream<List<Task>> streamMyTasks({required String familyId, required String userId}) {
    ...
    return _taskStreamController.stream
        .map((allTasks) => allTasks.where((task) => task.assigneeId == userId && task.familyId == familyId).toList())
        .handleError((e, s) {
          logger.e("MockTaskService: Error streaming tasks: $e", error: e, stackTrace: s);
        });
  }
  ...
  Future<void> updateTask({required String familyId, required Task task}) async {
    ...
    if (index != -1) {
      ...
      logger.w("MockTaskService: Task with ID ${task.id} not found for update.");
    }
  }

  @override
  Future<void> updateTaskStatus({required String familyId, required String taskId, required TaskStatus newStatus}) async {
    logger.d("Mock: Updating status for task $taskId to ${newStatus.name} for family $familyId.");
    final index = _tasks.indexWhere((t) => t.id == taskId && t.familyId == familyId);
    if (index != -1) {
      _tasks[index] = _tasks[index].copyWith(status: newStatus);
      _taskStreamController.add(List.from(_tasks)); // Notify listeners
    } else {
      logger.w("MockTaskService: Task with ID $taskId not found for status update.");
    }
  }

  @override
  Future<void> assignTask({required String familyId, required String taskId, required String assigneeId}) async {
    logger.d("Mock: Assigning task $taskId to $assigneeId for family $familyId.");
    final index = _tasks.indexWhere((t) => t.id == taskId && t.familyId == familyId);
    if (index != -1) {
      _tasks[index] = _tasks[index].copyWith(assigneeId: assigneeId, status: TaskStatus.assigned);
      _taskStreamController.add(List.from(_tasks)); // Notify listeners
    } else {
      logger.w("MockTaskService: Task with ID $taskId not found for assignment.");
    }
  }
  ...
  Stream<List<Task>> streamTasks({required String familyId}) {
    ...
    return _taskStreamController.stream
        .map((allTasks) => allTasks.where((task) => task.familyId == familyId).toList())
        .handleError((e, s) {
          logger.e("MockTaskService: Error streaming all tasks: $e", error: e, stackTrace: s);
        });
  }
  ...
  Stream<List<Task>> streamAvailableTasks({required String familyId}) {
    ...
    return _taskStreamController.stream
        .map((allTasks) => allTasks.where((task) => task.familyId == familyId && task.status == TaskStatus.available).toList())
        .handleError((e, s) {
          logger.e("MockTaskService: Error streaming available tasks: $e", error: e, stackTrace: s);
        });
  }

  @override
  Stream<List<Task>> streamTasksByAssignee({required String familyId, required String assigneeId}) {
    logger.d("Mock: Streaming tasks by assignee $assigneeId for family $familyId.");
    return _taskStreamController.stream
        .map((allTasks) => allTasks.where((task) => task.familyId == familyId && task.assigneeId == assigneeId).toList())
        .handleError((e, s) {
          logger.e("MockTaskService: Error streaming tasks by assignee: $e", error: e, stackTrace: s);
        });
  }

  @override
  Future<void> approveTask({required String familyId, required String taskId, required String approverId}) async {
    logger.d("Mock: Approving task $taskId for family $familyId by $approverId.");
    final index = _tasks.indexWhere((t) => t.id == taskId && t.familyId == familyId);
    if (index != -1) {
  ...
  Future<void> rejectTask({required String familyId, required String taskId, required String rejecterId, String? comments}) async {
    logger.d("Mock: Rejecting task $taskId for family $familyId by $rejecterId.");
    final index = _tasks.indexWhere((t) => t.id == taskId && t.familyId == familyId);
    if (index != -1) {
      _tasks[index] = _tasks[index].copyWith(status: TaskStatus.needsRevision);
      _taskStreamController.add(List.from(_tasks)); // Notify listeners
    } else {
      logger.w("MockTaskService: Task with ID $taskId not found for rejection.");
    }
  }

  @override
  Future<void> claimTask({required String familyId, required String taskId, required String userId}) async {
    logger.d("Mock: Claiming task $taskId by user $userId for family $familyId.");
    final index = _tasks.indexWhere((t) => t.id == taskId && t.familyId == familyId);
    if (index != -1) {
      _tasks[index] = _tasks[index].copyWith(assigneeId: userId, status: TaskStatus.assigned);
      _taskStreamController.add(List.from(_tasks)); // Notify listeners
    } else {
