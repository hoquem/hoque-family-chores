...
- **Idempotency:** the transaction re-reads the rule; a concurrent tick that already advanced `nextDueAt` is skipped. A per-rule error logs and is skipped; the tick continues. A structurally-broken rule (unparseable RRULE, missing template) is **disabled** (`enabled = false`) and logged once — a skipped-but-enabled rule would re-error every tick forever. (User ruling 2026-08-14, overrides the earlier "skip" wording.)
