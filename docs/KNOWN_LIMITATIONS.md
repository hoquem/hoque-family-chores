...
own `role` field is not protected the way the star economy assumes. Tracked as
**TASK-494** (private board); see "Open security items" in `PROJECT_STATUS.md`.
This repository is public, so the reproduction path is deliberately not written
down here.
